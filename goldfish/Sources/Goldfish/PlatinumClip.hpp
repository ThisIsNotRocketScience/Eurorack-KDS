...
CMachineParameter const paraPush =
{
	pt_byte,                                                                                // type
	"Push",	
	"Push",																		            // description
...
CMachineParameter const paraSound =
{
	pt_byte,                                                                                // type
	"Sound",
	"Sound",                                                                  // description
...
extern CMachineParameter const *ppcParameters[];
...
public:
	...
	bool Work(float *psamples, int numsamples, int const mode)
	{
		if(mode == WM_WRITE) {
			...
			memset(psamples, 0, numsamples * sizeof(float));
		}
		...
		if(mode == WM_READ || bypass) {
			reset();
			_active = false;
			return true;
		}

		//static const float hardclipgain = powf(10.0f, 4.8f / 20.0f);
		bool active = false;

		int32_t samplebuf[32];

		while(numsamples) {
			int n = numsamples > 32 ? 32 : numsamples;
			int i;

			numsamples -= n;

			// prepare for fixed-point processing
			for (i = 0; i < n; i++) {
				samplebuf[i] = int(psamples[i] * 256.0);
			}

			active = IntProcess(samplebuf, n);
			// finish fixed-point processing
			for (i = 0; i < n; i++) {
				psamples[i] = samplebuf[i] * (1.0 / 256.0);
			}
	...
	char const *DescribeValue(int const param, int const value)
	{
		...
		switch(param) {
		case 0:
			if(value == 1) return "Bypass";
			else return "Normal";
		case 1:
			//		_snprintf(txt, sizeof(txt), "%1.2f", value * (1.0 / 128.0) * 10.0);
			break;
		case 2:
	...
	//bool WorkStereoWrite(float *psamples, int numsamples);

	bool quiet()
	{
		return false;
		//return f1a.quiet() && f1b.quiet() && f2a.quiet() && f2b.quiet();
	}
