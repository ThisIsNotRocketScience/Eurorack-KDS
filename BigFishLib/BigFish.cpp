...
void BigFish_Filter(struct BigFish_t *fish, int32_t *bufferin, int32_t *bufferout, int len)
{
	...
	int32_t FilterEnvResult = ADSR_Get(&fish->FilterEnvelope, fish->SampleRate / len);
	...
	if (KeyTrack != FILTERKEYTRACK_OFF)
	{
		...
		float adsr = (ENV * FilterEnvScale * 127) / 32768.0f;
	...
	{		
		...
		float adsr = (ENV * FilterEnvScale * 20000) / 32768.0f;
		freq += adsr;
	}
...
void BigFish_GenerateBlock(struct BigFish_t *fish, int32_t *bufferOSCOUT, int32_t *bufferMAIN, int len)
{
	...
	while (len > 0)
	{
		...
		for (int i = 0; i < L; i++)
		{
			...
			AMPED[i] = (OSC[i]*AmpEnvelope) / 32768;
			*bufferOSCOUT++ = OSC[i];
		}
